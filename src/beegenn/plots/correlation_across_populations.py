            ...
            for k in events['tau']:
                for z in events['c']:
                    for w in events['amplitude']:
                        ...
                        res[f"pn_{start_time}_{end_time}"] = data
                        res[f"ln_{start_time}_{end_time}"] = data
                        res[f"orn_{start_time}_{end_time}"] = data
                        start_time += 120000.0
                        end_time += 120000.0
    return res
...
if __name__ == "__main__":
    ...
    for i in data:
        for j in data:
            if i.split('_')[1:] == j.split('_')[1:] and i.split('_')[0] != j.split('_')[0]:
                print(i.split('_')[1:], i.split('_')[0], j.split('_')[0], data[i]['mean'] - data[j]['mean'])
