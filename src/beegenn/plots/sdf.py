...
def plot_sdf_heatmap_per_pop(sdf_average, t_start, t_end, dt, pop, subplot):
    ...
    subplot.xaxis.set_major_locator(
        matplotlib.ticker.FixedLocator(
            [3000 * i // dt for i in range(int(t_end - t_start) // 3000 + 1)]
        )
    )
    subplot.set_xticklabels(
        [f"{t_start * (i + 1):.1g}" for i in range(int(t_end - t_start) // 3000 + 1)],
        rotation=45,
        fontsize=8,
    )
...
def get_subplots(n_pops):
    figure, subplots = plt.subplots(1, n_pops, sharey=True, layout="constrained")
...
def plot_sdf_heatmap(pops, t_start, t_end, data_manager, show):
    ...
    for pop, subplot in zip(pops, subplots):
        sdf_avg = data_manager.sdf_per_glomerulus_avg(pop, t_start, t_end)
        image.append(
...
if __name__ == "__main__":
    ...
    events = pd.read_csv(
        Path(param["simulations"]["simulation"]["output_path"])
        / param["simulations"]["name"]
        / "events.csv"
    )
